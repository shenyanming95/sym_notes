...
永远需要明确一点，一个CPU（单核）在一个瞬间，只能执行一个进程。但CPU可以在一秒内，快速切换不同的进程执行，使每个进程各运行几十或几百个毫秒，这样就产生并行的错觉，也被称为“伪并行”。这种快速的切换称作多道程序设计！
...
如果有进程执行系统调用被阻塞了，就会将进程的`task_struct`移出运行队列，添加到等待队列，并且将进程的状态的置为`TASK_UNINTERRUPTIBLE`或者`TASK_INTERRUPTIBLE`，重新触发一次 CPU 调度让出 CPU。进程加入等待队列的同时，还会向内核注册一个中断程序（回调函数）。例如：如果阻塞在Socket，那么当网卡接收到数据时，网卡就会产生硬件中断，内核再通过调用中断程序唤醒进程，将相应进程`task_struct`的状态置为`TASK_RUNNING`，并重新加入到运行队列。所以可以看到，在足够长的一段时间后，所有的进程都运行了，但在任何一个给定的瞬间仅有一个进程真正在运行
...
内核线程（Kernel-Level Thread），简称KLT，就是直接由操作系统内核创建和管理的线程。内核通过操纵调度器（Scheduler）对KLT进行调度来实现线程切换，并负责将KLT的任务映射到各个CPU上。但是程序并不会直接使用内核线程（KLT），而是使用内核线程的一种高级接口  — 轻量级进程（Light Weight Process，LWP），轻量级进程就是我们通常意义上所讲的线程。每个LWP都由一个内核线程支持，因此只有先支持内核线程，才能有轻量级进程。这种轻量级进程与内核线程之间1：1的关系称为一对一的线程模型：

![](./images/轻量级进程与内核线程之间一比一的关系.png)

这种设计：

- 好处：进程中的线程LWP都有一个KLT对应，当某个LWP因为系统调用阻塞了，不会影响进程的其它线程；
- 坏处：由于LWP基于KLT，所以在创建、析构及同步它的时候，都需要进行系统调用，就会涉及用户态和内核态的切换，其代价比较高；而且KLT需要维护LWP，会占用内核空间
...
| 状态名称     | 说明                                                         |
| ------------ | ------------------------------------------------------------ |
| NEW          | 初始状态，线程被构建，但是还未调用start()方法启动            |
| RUNNABLE     | 运行状态，Java线程将OS中的`就绪`和`运行`两种状态统称为“运行中” |
| BLOCKED      | 阻塞状态，表示线程阻塞与锁                                   |
| WAITING      | 等待状态，表示线程进入等待状态，那么当前线程需要等待其它线程通知or中断，才会重新运行 |
| TIME_WAITING | 超时等待状态，在 WAITING状态基础上增加等待时间控制，可以在指定时间内自行返回 |
...
Java启动线程都需要通过`java.lang.Thread#run()`，其它方式最后都是调用到这里，所以Java里面启动线程有3种方式：

- 继承java.lang.Thread，重写run()方法实现线程逻辑，最后调用Thread.start()方法来启动线程；
- 实现Runnable，通过Thread构造函数把接口对象传值，再调用start()方法启用线程；
...
| 方法名                                                  | 作用                                                         |
| ------------------------------------------------------- | ------------------------------------------------------------ |
| join()                                                  | 将当前线程挂起，让出CPU资源给指定线程执行，直至指定线程执行完，再重新请求CPU资源执行 |
...
| interupt()、<br />isInterrupted()、<br />interruptted() | interrupt()可以标志线程的中断状态为true，它并不能中止线程的运行，有两种情况：a.对于阻塞的线程，调用interrupt()方法会立即抛出InterruptedException异常，并且清除此线程的中断状态；b.对于正在运行的线程，调用interrupt()方法只是标志它为中断状态，并不会停止线程的运行。<br /><br />isInterrupted()返回指定线程的中断状态，此方法不会清除线程的中断状态而已，它只是作为一个判断而已；<br /><br />Thread.interrupted()方法是获取当前线程的中断状态并且清除它的中断状态，意味着紧连两次调用它，第二次必定返回false |
| setUncaughtExceptionHandler()                           | 当线程发生未捕获异常时，又或者父线程想处理子线程的异常时，就可以给子线程设置一个回调处理器：java.lang.Thread.UncaughtExceptionHandler。当线程发生未经捕获的异常，JVM将调用Thread中的dispatchUncaughtException方法把异常传递给线程的未捕获异常处理器： |
...
Java内存模型规定了工作内存与主内存之间交互的协议，定义了8种原子操作。Java中所有共享变量都是以这种方式读取的：（以下8个操作每一个都是原子性）
...
- 不允许线程丢弃它最近的assign操作，即**变量的值在工作内存改变了，必须要将该变量改变后的值同步回主内存**中。
...
happen-before，即先行发生原则。何为先行发生？指的是JMM定义的两项操作之间的偏序关系。假设操作A和操作B，如果保证A先行发生于B，则A的操作对B是可观察的，包括修改共享数据，发送消息，调用方法等等。反之如果无法确保A和B哪个先行发生，则无法保障A和B之间的操作互相可观察。举个例子：
...
但是在线程C启动，即使确保线程A先行发生于线程B，但是B与C之间没有确定的先行发生关系，线程C有可能先于B执行，也有可能后于B执行，即线程C的操作可能被B观察到也可能不会，这就导致B可能读取到过期数据的风险，j的值可能为1，也可能为2。
...
- 多线程间可见性（某个线程修改了被volatile修饰的变量，其它线程可以实时收到最新值）
- 防止指令重排序

它是处于普通变量与加锁变量之间的维度，它不会像synchronized一样锁住变量。但是：<u>**volatile关键字对于基本类型的修改可以在随后对多个线程的读保持一致，但是对于引用类型如数组、实体bean，仅仅保证引用的可见性，但并不保证引用内容的可见性**</u>
...
按照指令重排序，第1步一定先执行(因为2,3步依赖于第1步)，但是2和3没有依赖关系，可能执行顺序会变为1-3-2，先赋值引用再进行初始化。在上面代码中，假设有2个线程在调用方法getSingleton()。如果没发生指令重排序，Thread1抢到锁进入方法为Singleton初始化，在第3步执行之前，singleton==null一直为true，此时thread2就会在synchronized那边等待锁资源；一旦发生了指令重排序,thread1执行完第1步后直接执行第3步，将内存地址赋值给引用，这时singleton\==null就会返回false，thread2就直接越过if判断，将对象返回，但此时对象仍在初始化，就极有可能抛出空指针异常。所以对于这种情况，应该为Singleton对象加上volatile修饰，防止其指令重排序，其原理是内存屏障！！！
...
ThreadLocal，线程本地变量，或者线程本地存储，它是区别于锁的一种线程安全实现方案。从Java官方文档中的描述： **ThreadLocal类用来提供线程内部的局部变量**。这种变量在多线程环境下访问（通过get和set方法访问）时能保证各个线程的变量相对独立于其他线程内的变量。 ThreadLocal实例通常来说都是private static类型的，用于关联线程和线程上下文。

![](./images/线程范围内数据共享实例.png)

它可以为每个线程拷贝一份共享变量的副本，这样线程之间的共享变量的值就是相互独立的。正是由于每一个线程都拥有自己的变量副本，从而也没有必要为该变量加锁了。但是由于ThreadLocal一次只能保存一个变量，对于有多个变量需要保存的，可以将变量定义在对象里，然后把这个对象保存到ThreadLocal中。
...
![](./images/ThreaadLocal与ThreadLocalMap关系.png)

### 6.1.1.ThreadLocalMap

ThreadLocalMap 是 ThreadLocal 的内部数据结构，它是一个散列表，本质就是数组，其中的每个元素是一个键值对，键是 ThreadLocal 对象，值是线程局部变量的值。与 HashMap 一样，ThreadLocalMap 也会存在哈希表常见的问题，比如：哈希函数的选择、[哈希冲突](https://zhuanlan.zhihu.com/p/29520044)、装载因子、扩容...等。其中，不同于 HashMap 使用“链地址法”解决哈希冲突，ThreadLocalMap 选择“**开放地址法**”。它的类定义如下：
...
static class ThreadLocalMap {
  ...
  static class Entry extends WeakReference<ThreadLocal<?>> {
    ...
    Entry(ThreadLocal<?> k, Object v) {
      // 键会被自动回收掉. 所以如果外部该ThreadLocal的引用断开了，那么这个ThreadLocal会在GC的时候
      // 被回收，此时保存在Entry的key就会变成null.
      super(k);
      value = v;
    }
  }
...
void createMap(Thread t, T firstValue) {
 	 //调用构造方法创建一个ThreadLocalMap, 然后赋值给Thread对象的threadLocals属性
...
private void set(ThreadLocal<?> key, Object value) {
    ...
    for (Entry e = tab[i]; e != null; e = tab[i = nextIndex(i, len)]) {
        ...
        ThreadLocal<?> k = e.get();
        // 同一个ThreadLocal对象, 执行修改将旧值覆盖, 方法返回.
        if (k == key) {
            e.value = value;
            return;
        }
        // 如果key为null, 说明ThreadLocal对象被回收了, 那就调用replaceStaleEntry()替换当前位置
        // 的Entry，然后方法返回
        if (k == null) {
...
private void replaceStaleEntry(ThreadLocal<?> key, Object value, int staleSlot) {
    ...
    // 找到 ThreadLocal 被回收的起始Entry位置，遍历到entry数组小端位置（staleSlot--）下的第一个
    // Entry为null为止。
    ...
    // 找到 ThreadLocal 被回收的起始Entry位置，遍历到entry数组大端位置（staleSlot++）下的第一个
    // Entry为null为止。
    for (int i = nextIndex(staleSlot, len); (e = tab[i]) != null; 
         i = nextIndex(i, len)) {
        ThreadLocal<?> k = e.get();
        // 当前ThreadLocal原来已经存在于Entry数组中。只有一种原因：当前ThreadLocal（简称为A），
        // 本来计算得到A应该放入的Entry[]数组下标假设为5，不巧的是，下标为5的这个位置上已经有元素了
        //（即hash冲突了）所以A就往后寻找位置，存放到下标为6的位置。然后因为其它原因，下标为5的Entry
        // 元素的key被回收了，此时再设置A，就会进入这个if分支（见下图）
        if (k == key) {
            ...
            // 等于staleSlot，这时候就要把原先下标为staleSlot的Entry对象给回收掉, 但是上面代码已
            // 经将它们互换位置了，所以此时应该被回收的Entry对象下标为i，将i赋值给slotToExpunge
        ...
        // 能进入这个if分支，说明此时的entry数组并没有当前要set进来的threadLocal对象A，那么 
        // staleSlot 位置就要用来存放A。slotToExpunge==staleSlot，说明在第一个for循环中并未找
        // 到下标在staleSlot之前的并且已经过期的Entry对象，那么需要 slotToExpunge 设置为向后遍历
        // 的第一个过期对象的位置
        if (k == null && slotToExpunge == staleSlot)
            slotToExpunge = i;
    }
    // 代码能到这里，说明原先的Entry[]数组就没有当前的ThreadLocal存放进去，所以就创建一个新的Entry
    // 对象放置到下标为staleSlot的位置上
...
![](./images/ThreadLocal-replaceStaleEntry.png)

#### 6.1.2.2.expungeStaleEntry()

上面的replaceStaleEntry()方法发现有过期的Entry，先会通过expungeStaleEntry()方法获取下标，它会返回 staleSlot 之后的下一个空槽的索引（staleSlot 和此槽之间的所有槽都将被检查是否已删除）然后再调用cleanSomeSlots()清理资源。expungeStaleEntry()方法的源码如下：

```java
// 源码：ThreadLocal - 589行
private int expungeStaleEntry(int staleSlot) {
    // 参数 staleSlot 就是需要回收的Entry元素在数组的下标
    Entry[] tab = table;
    int len = tab.length;

    // 将这个Entry置为null，等待GC线程回收它
    tab[staleSlot].value = null;
    tab[staleSlot] = null;
    size--;
    ...
    Entry e;
    int i;
    // 分析 replaceStaleEntry() 方法知道，staleSlot 是第一个待回收的Entry的数组下标，因此我们必
    // 须向后（staleSlot++）寻找，尽可能得找到那些应该回收的Entry
    for(i = nextIndex(staleSlot, len); (e = tab[i]) != null; i = nextIndex(i, len)) {
        ThreadLocal<?> k = e.get();
        if (k == null) {
            // kye为null，threadLocal被回收，把它的Entry都置为null, 在gc时回收掉, size减1
            e.value = null;
            tab[i] = null;
            size--;
        } else {
            // key不为null，通过哈希函数，计算它初始的Entry数组下标位置
            int h = k.threadLocalHashCode & (len - 1);
            if (h != i) {
                // 如果h和i不一样, 那么说明之前该ThreadLocal在放入Entry[]数组的时候必定发生了哈
                // 希冲突, 所以它实际处的位置不等于计算得到的理论位置. 那么就将它实际处的位置，置为
                // null（因为要准备将它向前面迁移了）
                tab[i] = null;
...
![](./images/ThreadLocal-expungeStaleEntry.png)

#### 6.1.2.3.cleanSomeSlots()

执行完expungeStaleEntry()方法获取遍历到的下标i，它是staleSlot往后找到不为null的元素，也有可能直接staleSlot+1的位置。接着就会调用cleanSomeSlots()，这个方法会在两个地方调用：一处是调用set()插入的时候可能会被调用，另一处是在替换无效slot（即调用expungeStaleEntry()方法）的时候可能会被调用，前者传入的n为元素个数，后者为table的容量。
...
private boolean cleanSomeSlots(int i, int n) {
    ...
    } while ( (n >>>= 1) != 0); // 然后n无符号右移1位, 类似于除以2，最多会执行log2(n)次的扫描
    return removed;
}
```
...
**内存泄漏**（英语：memory leak）是计算机科学中的一种资源泄漏，主因是计算机程序的内存管理失当，因而**失去对一段已分配内存空间的控制**，程序继续占用已不再使用的内存空间，或是存储器所存储之物件无法透过执行代码而访问，令内存资源空耗。站在Java程序的角度，就是我们没办法通过任何引用访问到变量，而GC线程又无法回收掉这个变量占据的内存空间，这个变量就造成了内存泄露。
...
1. 一个对象，**有且仅有一个引用**，并且这个引用是弱引用，它才会被垃圾回收；如果这个对象，既有强引用，又有弱应用，在GC的时候，它是不会被回收的。
2. WeakReference引用本身是强引用，**它内部的（T reference）才是真正的弱引用字段**，WeakReference就是一个装弱引用的容器而已。

### 6.2.2.泄露的本质

ThreadLocal造成内存泄露的核心原因，就是 ThreadLocal 强引用断开，导致该对象只存在 ThreadLocalMap.Entry 到 ThreadLocal 的弱应用，那么它就会在下次GC中被回收掉；而它对应的存在 ThreadLocalMap 中的 value 由于强引用链关系而不是被GC线程回收。最终导致程序没有入口可以访问到这个 value，但是它又不能被回收，从而造成内存泄露。

![](./images/ThreadLocal内存泄露.png)

因此，在实际开发中，我们常常会把 ThreadLocal 定义成 private static 的访问级别，这样声明后，此时ThreadLocal由于存在强引用，除非对应的ClassLoader被回收，否则不会被GC回收。但是这种处理就可以一劳永逸吗？很明显不能，在生产代码中，线程的创建和销毁都会耗费CPU资源，我们都会使用线程池来管理线程，线程池内部是复用线程来执行任务的。此时如果任务执行完，ThreadLocal由于被强引用不会回收，它就会随着线程进入下一个任务，从某种意义上说这也是一种“内存泄露”。

## 6.3.扩展点

### 6.3.1.对比Synchronized
...
不管是 ThreadLocal 还是 InheritableThreadLocal，都无法再线程池中使用。举个例子，在一个RPC框架中，比如dubbo，它底层使用的是 netty 的 worker 线程组，我们一般把他们叫做 I/O 线程。RPC框架是需要传递上下文对象的，比如 traceId（它是实现分布式链路追踪的关键）大部分框架实现都会把 traceId 存到 ThreadLocal 中。如果业务逻辑使用了异步调用，由于线程池的线程并不全是创建的，此时的 traceId 存放在 I/O 线程上，实际逻辑执行在业务线程上，traceId 就会丢失，导致调用链路不完整。针对这种场景，我们一般会使用阿里开源的[TransmittableThreadLocal](https://github.com/alibaba/transmittable-thread-local)来解决。

![](./images/TransmittableThreadLocal.png)

- ThreadLocal适用于在单个线程内共享变量的场景；
- InheritableThreadLocal适用于父子线程之间传递变量的场景；
- TransmittableThreadLocal适用于多线程环境下传递变量的场景，并且能够正确处理线程池等复杂情况。
...
| 方法名称        | 描述                                                         |
| --------------- | ------------------------------------------------------------ |
| notify()        | 通知一个在对象上等待的线程，使其从wait()方法返回，返回的前提是线程必须获取该对象的锁 |
| notifyAll()     | 通知所有等待在该对象上的线程                                 |
| wait()          | 调用此方法的线程进入WAITING状态，只有等待其它线程通知或被中断后才返回，调用wait()会释放对象的锁 |
| wait(long)      | 超时等待一段时间，参数单位是毫秒，如果时间到还没被通知则超时返回 |
