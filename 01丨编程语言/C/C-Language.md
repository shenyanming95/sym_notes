...
一般，调用`fork()`函数的进程称为父进程，被创建的新进程，称其为子进程。在调用`fork()`函数后，会返回两次，一次在父进程中，一次在子进程中，这样就实现了 fork() 函数的“两次返回”！！返回值是一个`pid_t`，它指示进程的类型，如果返回值为0，则表示当前进程为子进程；如果返回值大于0，则表示当前进程为父进程，返回值是子进程的进程ID；返回值为-1表示异常。
