...
进程（process），本质上是正在执行的一个程序，它是操作系统分配资源的最小单位。与每个进程相关的是进程的地址空间（address space），这是从某个最小值的存储位置（通常是零）到某个最大值存储位置的列表，该地址空间中存放有可执行程序、程序的数据以及程序的堆栈。
...
图a表示：一台多道程序计算机的内存中有4道程序；

图b表示：每道程序都各自控制自己的流程（即它们拥有各自的程序逻辑计数器），但实际上，只有一个物理程序计数器。那就意味着，当程序A执行时，它将自己的逻辑程序计数器装入实际的物理程序计数器中，等到A执行完，再将物理程序计数器保存到内存中该进程的逻辑程序计数器；

图c表示：足够长的一段时间后，所有的进程都运行了，但在任何一个给定的瞬间仅有一个进程真正在运行
